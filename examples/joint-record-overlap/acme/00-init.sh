#!/usr/bin/env bash
# Acme (data owner / keysetup lead) session setup.
#
# Run once per machine, per collaboration. Walks you through:
#   1. Picking a collaboration where you're the data owner.
...
step "JuLenny demo session setup (Acme: data owner / lead)"
...
# -------- Pick collaboration --------
# A collaboration (project) is a (you, partner, function-set) tuple with a
# shared joint key. One collaboration can hold many permissions (permissions).
# We list only collaborations where you're the data owner; collaborations
# where you're the partner belong to the beta/ folder.
step "Fetching your collaborations..."
ALL_PROJECTS="$(list_collaborations)"
OWNED_PROJECTS="$(echo "$ALL_PROJECTS" | jq '[.[] | select(.ownerCompanyName == null)]')"
PROJECT_COUNT="$(echo "$OWNED_PROJECTS" | jq 'length')"

if (( PROJECT_COUNT == 0 )); then
    err "No collaborations found where you are the data owner."
    err "Create one via the web UI at /company/collaborate/new, then re-run this script."
    exit 1
fi
...
echo "$OWNED_PROJECTS" \
    | jq -r 'to_entries[] | "  [\(.key + 1)] \(.value.name // "(unnamed)")  |  partner: \(.value.partnerCompanyName // .value.partnerCompanyId)  |  \(.value.permissionCount) permission(s)  |  keysetup: \(.value.keysetupState // "n/a")  |  created \(.value.createdAt // "?" | .[0:10])"'
echo
...
if (( PERMISSION_COUNT == 0 )); then
    ...
    err "Create one via the web UI (Add Permission button on the collaboration page), then re-run."
    exit 1
fi

# Sort by createdAt descending so newest is [1].
...
JULENNY_COMPANY_ID="$(echo "$PERM_OBJ" | jq -r '.dataOwnerCompanyId')"
JULENNY_PEER_COMPANY_ID="$(echo "$PERM_OBJ" | jq -r '.dataConsumerCompanyId')"
...
[[ -n "$JULENNY_COMPANY_ID" && "$JULENNY_COMPANY_ID" != "null" ]] \
    || die "Selected permission has no dataOwnerCompanyId."
...
info "  Acme is:     data owner / keysetup lead ($JULENNY_COMPANY_ID)"
info "  Beta (peer): data consumer / keysetup main ($JULENNY_PEER_COMPANY_ID)"
...
# -------- Write config.env --------
cat > "$JL_CONFIG" <<EOF
# JuLenny demo session config for Acme (data owner / lead).
