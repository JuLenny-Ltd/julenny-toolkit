#!/usr/bin/env bash
# Shared helpers for the Acme (data owner / lead) demo scripts.
...
wait_msg() {
    echo
    echo "$(_yellow '────────────────────────────────────────────────')"
    echo "$(_yellow '  WAITING ON THE OTHER SIDE (Beta)')"
    echo "$(_yellow '────────────────────────────────────────────────')"
    echo "  $*"
    echo
}
...
download_peer_share() {
    ...
    [[ -n "$msg" && "$msg" != "null" ]] \
        || die "Beta has not yet submitted a '$msg_type' share. Tell them to run the previous step."
...
wait_for_peer_share() {
    # Block until the peer has uploaded a message of the given type, with backoff.
    local msg_type="$1"
    local max_wait_sec="${2:-1800}"  # 30 min default
    local peer; peer="$(peer_company_id)"

    local elapsed=0
    local delay=5
    info "Waiting for Beta ($peer) to submit '$msg_type'..."
    while (( elapsed < max_wait_sec )); do
        ...
        if echo "$resp" | jq -e --arg t "$msg_type" '.messages[]? | select(.messageType == $t)' > /dev/null 2>&1; then
            success "Beta submitted '$msg_type'."
            return 0
        fi
        sleep "$delay"
    ...
    die "Timed out after ${max_wait_sec}s waiting for Beta to submit '$msg_type'."
}
...
prompt_for() {
    # Usage: prompt_for VAR_NAME "prompt text" [default]
    local var_name="$1"
    local prompt_text="$2"
    local default_val="${3:-}"
    local value
...
# filter client-side by id. Acme defaults to view=permissioned; pass view=received
# explicitly when needed.
fetch_permission() {
    local perm_id="${1:-$JULENNY_PERMISSION_ID}"
    local view="${2:-permissioned}"
    local resp
...
# Acme is the data owner, so defaults to view=permissioned.
