#!/usr/bin/env bash
# decision-tree-inference, DATA-OWNER (Acme) side. Thin scenario bootstrap:
# selects our side + this scenario's data dir, then hands off to the shared
# _core driver. Acme provides the 'features' input (packed-real); the exact
# function/version is picked at 00-init time.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
export JULENNY_OUR_SIDE="data-owner"
