#!/usr/bin/env bash
# decision-tree-inference, DATA-CONSUMER (Beta) side. Thin scenario bootstrap:
# selects our side + this scenario's data dir, then hands off to the shared
# _core driver. Beta owns the 'model' input (encrypted decision tree); the
# exact function/version is picked at 00-init time.
set -euo pipefail
