...
# Idempotence guard: a permission whose finalKeys are already registered
# rejects upload-url with a "keysetup is in state 'complete'" error. The local
# marker above normally catches this, but if it's missing (e.g. a prior run
# finalized but didn't reach the touch), detect the already-final state here
# and skip gracefully instead of dying.
